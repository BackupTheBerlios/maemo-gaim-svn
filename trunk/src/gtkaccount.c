...
#if defined (GAIM_SMALL_SCREEN) && !defined(ENABLE_HILDON)
enum
{
    COLUMN_AUTOLOGIN,
    COLUMN_SCREENNAME,
...
typedef struct
{
    ...
    GtkWidget *vbox_user;
    GtkWidget *vbox_protocol;
    GtkWidget *vbox_login_top;
    GtkWidget *vbox_user_top;
    GtkWidget *vbox_proxy_top;
    GtkWidget *vbox_protocol_top;
#endif
...
#else
    add_login_options(dialog,    dialog->vbox_login);
    add_user_options(dialog,     dialog->vbox_user);
    add_protocol_options(dialog, dialog->vbox_protocol);
#endif
...
add_login_options(AccountPrefsDialog *dialog, GtkWidget *parent)
{
    ...
    if (dialog->vbox_login_top != NULL)
        gtk_widget_destroy(dialog->vbox_login_top);

    /* Main vbox */
    dialog->vbox_login_top = vbox = gtk_vbox_new(FALSE, CONTAINER_BORDER_WIDTH);
...
add_user_options(AccountPrefsDialog *dialog, GtkWidget *parent)
{
    ...
    if (dialog->vbox_user_top != NULL)
        gtk_widget_destroy(dialog->vbox_user_top);
    
    /* Main vbox */
    dialog->vbox_user_top = vbox1 = gtk_vbox_new(FALSE, CONTAINER_BORDER_WIDTH);
    gtk_container_add(GTK_CONTAINER(parent), vbox1);
    gtk_widget_show(vbox1);
    
    /* Build the user options frame. */
    frame = gaim_gtk_make_frame(vbox1, _("User Options"));
	...
	/* Buddy icon */
	dialog->icon_hbox = hbox = gtk_hbox_new(FALSE, CONTAINER_BORDER_WIDTH);
	...
	hbox2 = gtk_hbox_new(FALSE, CONTAINER_BORDER_WIDTH);
...
#else
    if (dialog->vbox_protocol_top != NULL)
        gtk_widget_destroy(dialog->vbox_protocol_top);

    /* Main vbox */
    dialog->vbox_protocol_top = vbox1 = gtk_vbox_new(FALSE, CONTAINER_BORDER_WIDTH);
    gtk_container_add(GTK_CONTAINER(parent), vbox1);
    gtk_widget_show(vbox1);
    
    frame = gaim_gtk_make_frame(vbox1, buf);
...
add_proxy_options(AccountPrefsDialog *dialog, GtkWidget *parent)
{
    ...
    if (dialog->vbox_proxy_top != NULL)
        gtk_widget_destroy(dialog->vbox_proxy_top);
    
    /* Main vbox */
    dialog->vbox_proxy_top = vbox1 = gtk_vbox_new(FALSE, CONTAINER_BORDER_WIDTH);
    gtk_container_add(GTK_CONTAINER(parent), vbox1);
    gtk_widget_show(vbox1);
    
    frame = gaim_gtk_make_frame(vbox1, _("Proxy Options"));
	...
	dialog->proxy_vbox = vbox2 = gtk_vbox_new(FALSE, CONTAINER_BORDER_WIDTH);
...
cancel_account_prefs_cb(GtkWidget *w, AccountPrefsDialog *dialog)
{
    ...
    gtk_container_remove(GTK_DIALOG(accounts_window->window)->vbox, dialog->main_vbox);
    gtk_container_add(GTK_DIALOG(accounts_window->window)->vbox, accounts_window->top_container);
    g_object_unref(accounts_window->top_container);
    gtk_widget_show(accounts_window->window);
#else
...
ok_account_prefs_cb(GtkWidget *w, AccountPrefsDialog *dialog)
{
	...
	account_win_destroy_cb(NULL, NULL, dialog);
#else
    /* call the same actions as a cancel */
							 ...
							 GaimAccount *account)
{
	...
	GtkWidget *button;
#ifdef GAIM_SMALL_SCREEN
    GtkWidget *account_pref;
    GtkWidget *vbox_user_options;
    GtkWidget *user_option_sw;
    GtkWidget *user_option_viewport;
    GtkWidget *lbl_login_options;
    GtkWidget *advanced_option_sw;
    GtkWidget *viewport;
    GtkWidget *lbl_advanced_options;
    GtkWidget *protocol_option_sw;
    GtkWidget *protocol_viewport;
    GtkWidget *protocol_vbox;
    GtkWidget *lbl_protocol_options;
#endif
...
#ifdef GAIM_SMALL_SCREEN
    dialog->window = win = accounts_window->window;
    g_object_ref(accounts_window->top_container);
    gtk_container_remove(GTK_DIALOG(win)->vbox, accounts_window->top_container);
#else
	...
	/* Setup the main vbox */
	dialog->main_vbox = main_vbox = gtk_vbox_new(FALSE, 0);
#ifdef GAIM_SMALL_SCREEN
	gtk_container_add(GTK_CONTAINER(GTK_DIALOG(win)->vbox), main_vbox);
#else
	...
	dialog->top_vbox = vbox = gtk_vbox_new(FALSE, BOX_SPACING);
...
#ifdef GAIM_SMALL_SCREEN
    account_pref = gtk_notebook_new ();
    gtk_widget_show (account_pref);
    gtk_box_pack_start (GTK_BOX (vbox), account_pref, TRUE, TRUE, 0);
  
    /* user login option */    

    user_option_sw = gtk_scrolled_window_new (NULL, NULL);
    gtk_widget_show (user_option_sw);
    gtk_container_add (GTK_CONTAINER (account_pref), user_option_sw);
    gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (user_option_sw), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
    
    user_option_viewport = gtk_viewport_new (NULL, NULL);
    gtk_widget_show (user_option_viewport);
    gtk_container_add (GTK_CONTAINER (user_option_sw), user_option_viewport);
    gtk_viewport_set_shadow_type (GTK_VIEWPORT (user_option_viewport), GTK_SHADOW_NONE);
        
    lbl_login_options = gtk_label_new (_("Login Options"));
    gtk_widget_show (lbl_login_options);
    gtk_notebook_set_tab_label (GTK_NOTEBOOK (account_pref), gtk_notebook_get_nth_page (GTK_NOTEBOOK (account_pref), 0), lbl_login_options);
    
    vbox_user_options = gtk_vbox_new (FALSE, 4);
    gtk_widget_show (vbox_user_options);
    gtk_container_add (GTK_CONTAINER (user_option_viewport), vbox_user_options);
    gtk_container_set_border_width (GTK_CONTAINER (user_option_viewport), 5);
    
    /* advanced option tab */    
    advanced_option_sw = gtk_scrolled_window_new (NULL, NULL);
    gtk_widget_show (advanced_option_sw);
    gtk_container_add (GTK_CONTAINER (account_pref), advanced_option_sw);
    gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (advanced_option_sw), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
    
    
    viewport = gtk_viewport_new (NULL, NULL);
    gtk_widget_show (viewport);
    gtk_container_add (GTK_CONTAINER (advanced_option_sw), viewport);
    gtk_viewport_set_shadow_type (GTK_VIEWPORT (viewport), GTK_SHADOW_NONE);
  
    lbl_advanced_options = gtk_label_new (_("Advanced Options"));
    gtk_widget_show (lbl_advanced_options);
    gtk_notebook_set_tab_label (GTK_NOTEBOOK (account_pref), gtk_notebook_get_nth_page (GTK_NOTEBOOK (account_pref), 1), lbl_advanced_options);
    
    dialog->bottom_vbox = dbox = gtk_vbox_new(FALSE, 4);
    gtk_container_add (GTK_CONTAINER (viewport), dbox);
    gtk_container_set_border_width (GTK_CONTAINER (dbox), 5);
    
    add_login_options(dialog, vbox_user_options);
    add_user_options(dialog, dbox);
    add_proxy_options(dialog, dbox);
    gtk_widget_show(dbox);
    
    /* protocol options tab */
    protocol_option_sw = gtk_scrolled_window_new (NULL, NULL);
    gtk_widget_show (protocol_option_sw);
    gtk_container_add (GTK_CONTAINER (account_pref), protocol_option_sw);
    gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (protocol_option_sw), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
    
    protocol_viewport = gtk_viewport_new (NULL, NULL);
    gtk_widget_show (protocol_viewport);
    gtk_container_add (GTK_CONTAINER (protocol_option_sw), protocol_viewport);
    gtk_viewport_set_shadow_type (GTK_VIEWPORT (protocol_viewport), GTK_SHADOW_NONE);
    
    lbl_protocol_options = gtk_label_new (_("Protocol Options"));
    gtk_widget_show (lbl_protocol_options);
    gtk_notebook_set_tab_label (GTK_NOTEBOOK (account_pref), gtk_notebook_get_nth_page (GTK_NOTEBOOK (account_pref), 2), lbl_protocol_options);
      
    protocol_vbox = gtk_vbox_new(FALSE, 4);
    gtk_container_add (GTK_CONTAINER (protocol_viewport), protocol_vbox);
    gtk_container_set_border_width (GTK_CONTAINER (protocol_vbox), 5);
    
    /* now add the protocol info */    
    add_protocol_options(dialog, protocol_vbox);
    gtk_widget_show(protocol_vbox);
    
    dialog->vbox_login = vbox_user_options;
    dialog->vbox_user = dbox;
    dialog->vbox_protocol = protocol_vbox;
#else
    ...
    dialog->bottom_vbox = dbox = gtk_vbox_new(FALSE, BOX_SPACING);
...
gaim_gtk_accounts_window_show(void)
{
    ...
    dialog->top_container = vbox = gtk_vbox_new(FALSE, BOX_SPACING);
#ifdef GAIM_SMALL_SCREEN
    ...
    gtk_box_set_spacing(GTK_BOX(bbox1), CONTAINER_BORDER_WIDTH);
    gtk_button_box_set_layout(GTK_BUTTON_BOX(bbox1), GTK_BUTTONBOX_END);
    gtk_box_pack_start(GTK_BOX(vbox), bbox1, FALSE, TRUE, 0);
...
#ifdef GAIM_SMALL_SCREEN
    gtk_box_pack_start(GTK_BOX(bbox1), button, FALSE, FALSE, 0);
#else
    gtk_box_pack_start(GTK_BOX(bbox), button, FALSE, FALSE, 0);
#endif
	gtk_widget_show(button);
