...
#define PROXYPASS 3
    ...
    #define PREF_UI_INIT() \
        GtkWidget *label; \
        ret = gtk_vbox_new(FALSE, BOX_SPACING); \
        gtk_container_set_border_width (GTK_CONTAINER (ret), CONTAINER_BORDER_WIDTH); 
#endif

#ifdef GAIM_SMALL_SCREEN
#define PREF_UI_MAKE_FRAME(x, y) \
    vbox = gaim_gtk_make_frame (ret, ""); \
    label = gtk_label_new (_(x)); \
    gtk_widget_show (label); \
    gtk_notebook_set_tab_label (GTK_NOTEBOOK (notebook), gtk_notebook_get_nth_page (GTK_NOTEBOOK (notebook), y), label); 
#else
#define PREF_UI_MAKE_FRAME(x, y) \
    vbox = gaim_gtk_make_frame (ret, _(x)); 
#endif

#define PREF_TOP_CONTAINER(x, y) \
    x = gtk_vbox_new(FALSE, BOX_SPACING);\
    gtk_container_set_border_width (GTK_CONTAINER (x), CONTAINER_BORDER_WIDTH);\
    y = x;
...
static GtkWidget* prev_app_view = NULL;
#endif
...
GtkWidget *interface_page() {
	GtkWidget *ret;
	GtkWidget *vbox;
    PREF_TOP_CONTAINER(ret, vbox);
	vbox = gaim_gtk_make_frame(vbox, _("Interface Options"));
...
GtkWidget *theme_page() {
	...
	GtkTreePath *path = NULL;
	/*GtkWidget *label; */ /* defined in PREF_UI_INIT macro */
	GtkTargetEntry te[3] = {{"text/plain", 0, 0},{"text/uri-list", 0, 1},{"STRING", 0, 2}};
    PREF_UI_INIT();
	
    PREF_UI_MAKE_FRAME("Smiley Themes", 0);
...
GtkWidget *messages_page() {
	...
	PREF_UI_MAKE_FRAME("Default Formatting", 2);
...
GtkWidget *hotkeys_page() {
    ...
    PREF_UI_MAKE_FRAME("Send Message", 0);
...
GtkWidget *list_page() {
    ...
    PREF_UI_MAKE_FRAME("Buddy List Sorting", 0)
    ...
    PREF_UI_MAKE_FRAME("Buddy List Window", 1)
    ...
    PREF_UI_MAKE_FRAME("Buddy Display", 2)
...
GtkWidget *conv_page() {
	...
	PREF_UI_MAKE_FRAME("Conversations", 0);
	...
	/* All the tab options! */
    PREF_UI_MAKE_FRAME("Tab Options", 1);
...
GtkWidget *network_page() {
	...
	GtkWidget *table, *auto_ip_checkbox, *ports_checkbox, *spin_button;
    /*GtkWidget *label;*/ /* defined in PREF_UI_INIT */
	GtkSizeGroup *sg;
	GaimProxyInfo *proxy_info;
    PREF_UI_INIT();

    PREF_UI_MAKE_FRAME("IP Address", 0);
    ...
    PREF_UI_MAKE_FRAME("Proxy Server", 2);
	...
	gtk_container_set_border_width(GTK_CONTAINER(table), CONTAINER_BORDER_WIDTH);
	gtk_table_set_col_spacings(GTK_TABLE(table), CONTAINER_BORDER_WIDTH);
	gtk_table_set_row_spacings(GTK_TABLE(table), BOX_SPACING);
	...
	hbox = gtk_hbox_new(TRUE, CONTAINER_BORDER_WIDTH);
...
GtkWidget *logging_page() {
    ...
    syslog_enabled = gaim_prefs_get_bool("/core/logging/log_system");

    PREF_UI_MAKE_FRAME("Message Logs", 0);
    ...
    PREF_UI_MAKE_FRAME("System Logs", 1);
...
/*	GtkWidget *label; */ /* defined in PREF_UI_INIT() macro */
    ...
    PREF_UI_INIT();
	sg = gtk_size_group_new(GTK_SIZE_GROUP_HORIZONTAL);
    PREF_UI_MAKE_FRAME("Sound Options", 0);
...
#ifndef _WIN32
	PREF_UI_MAKE_FRAME("Sound Method", 1);
	...
	hbox = gtk_hbox_new(FALSE, CONTAINER_BORDER_WIDTH);
	gtk_box_pack_start(GTK_BOX(vbox), hbox, FALSE, FALSE, CONTAINER_BORDER_WIDTH);

	label = gtk_label_new_with_mnemonic(_("Sound c_ommand:\n(%s for filename)"));
	gtk_size_group_add_widget(sg, label);
	gtk_misc_set_alignment(GTK_MISC(label), 0, 0);
	gtk_box_pack_start(GTK_BOX(hbox), label, FALSE, FALSE, CONTAINER_BORDER_WIDTH);
	...
	gtk_box_pack_start(GTK_BOX(hbox), entry, TRUE, TRUE, CONTAINER_BORDER_WIDTH);
	g_signal_connect(G_OBJECT(entry), "changed",
    ...
    PREF_UI_MAKE_FRAME("Sound Events", 2);
	...
	gtk_box_pack_start(GTK_BOX(hbox), sound_entry, FALSE, FALSE, CONTAINER_BORDER_WIDTH);
...
GtkWidget *away_page() {
	...
	PREF_UI_MAKE_FRAME("Auto-away", 2);
...
static GtkWidget *plugin_page ()
{
	...
	gtk_misc_set_padding(GTK_MISC(plugin_description), CONTAINER_BORDER_WIDTH, CONTAINER_BORDER_WIDTH);
	...
	gtk_misc_set_padding(GTK_MISC(plugin_details), CONTAINER_BORDER_WIDTH, CONTAINER_BORDER_WIDTH);
...
#ifdef ENABLE_HILDON
static gint prefs_close_cb(GtkWidget *w, gpointer user_data)
{
    current_app_view = prev_app_view;
    hildon_app_set_appview(HILDON_APP(app), HILDON_APPVIEW(current_app_view));
...
void gaim_gtk_prefs_show(void)
{
    ...
    if (prefs) {
#ifdef ENABLE_HILDON
        hildon_app_set_appview(HILDON_APP(app), HILDON_APPVIEW(prefs));
        current_app_view = prefs; 
#else
	...
	/* Create the window */
        ...
        current_app_view = prefs = hildon_appview_new( _("Preferences"));
        hildon_app_set_appview(HILDON_APP(app), HILDON_APPVIEW(prefs));
        gtk_widget_show(prefs);
    #else
        prefs = gtk_dialog_new();
        gtk_window_set_modal(GTK_WINDOW(prefs), TRUE);
        gtk_dialog_set_has_separator(prefs, FALSE);
    #endif
/*
#ifdef ENABLE_HILDON
    gtk_widget_set_size_request(prefs, 600, 500);
#endif
*/
#else
	...
	gtk_container_set_border_width(GTK_CONTAINER(prefs), CONTAINER_BORDER_WIDTH);
#endif
	...
	*/
    ...
    g_signal_connect_swapped(G_OBJECT(button), "clicked",
                             G_CALLBACK(prefs_close_cb), prefs);
#else
	g_signal_connect_swapped(G_OBJECT(button), "clicked",
...
#ifdef GAIM_SMALL_SCREEN
    gaim_prefs_add_int("/gaim/gtk/accounts/dialog/width",  420);
    gaim_prefs_add_int("/gaim/gtk/accounts/dialog/height", 275);
#else 
