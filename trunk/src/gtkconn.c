			   ...
			   GtkWidget *table, gint *rows)
{
	...
	gtk_table_attach (GTK_TABLE (table), meter->button, 3, 4, *rows, *rows+1, (GtkAttachOptions) (GTK_FILL), (GtkAttachOptions) (GTK_EXPAND), 0, 0);
...
gaim_gtk_connection_report_disconnect(GaimConnection *gc, const char *text)
{
	...
	if (!disconnect_window) {
		...
		disconnect_window->window = gtk_dialog_new_with_buttons(GAIM_ALERT_TITLE, NULL, GTK_DIALOG_MODAL | GTK_DIALOG_NO_SEPARATOR, NULL);
