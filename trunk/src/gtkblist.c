...
GaimGtkBuddyList *gtkblist = NULL;
#ifdef ENABLE_HILDON
...
rebuild_joinchat_entries(GaimGtkJoinChatData *data)
{
	...
	for (tmp = list; tmp; tmp = tmp->next)
	{
		...
		rowbox = gtk_hbox_new(FALSE, CONTAINER_BORDER_WIDTH);
...
#ifdef ENABLE_HILDON
    gtk_widget_set_size_request(GTK_WINDOW (data->window), 500, 320);
#endif

#endif
	gtk_dialog_set_default_response(GTK_DIALOG(data->window), GTK_RESPONSE_OK);
	gtk_container_set_border_width(GTK_CONTAINER(data->window), BOX_SPACING);
	gtk_window_set_resizable(GTK_WINDOW(data->window), FALSE);
	gtk_box_set_spacing(GTK_BOX(GTK_DIALOG(data->window)->vbox), CONTAINER_BORDER_WIDTH);
	gtk_container_set_border_width(
		GTK_CONTAINER(GTK_DIALOG(data->window)->vbox), BOX_SPACING);
	gtk_window_set_role(GTK_WINDOW(data->window), "join_chat");

#ifdef GAIM_SMALL_SCREEN    
    sw = gtk_scrolled_window_new (NULL, NULL);
    gtk_widget_show (sw);
    gtk_container_add (GTK_CONTAINER (GTK_DIALOG(data->window)->vbox), sw);
...
#else
  gtkblist->window = app;
  current_app_view = app_buddy_view = hildon_appview_new("");
  hildon_app_set_appview(HILDON_APP(app), HILDON_APPVIEW(app_buddy_view));
  gtk_window_set_title(GTK_WINDOW(gtkblist->window), _("gaim"));
#endif

#ifdef GAIM_SMALL_SCREEN 
    gtkblist->notebook = notebook = gtk_notebook_new ();
    
    /* vbox to main window */
    vbox = gtkblist->vbox = gtk_vbox_new(FALSE, 0);
    
    /* vbox for the notebook */
/*
    vbox = gtk_vbox_new(FALSE, 0);
*/
    /* use this vbox for the treeview and the lower icons */
#else
								 ...
								 const char *group, const char *alias)
{
...
#ifdef GAIM_SMALL_SCREEN
    gtk_window_set_modal (GTK_WINDOW (data->window), TRUE);
    gtk_widget_set_size_request(GTK_WINDOW (data->window), 420, 400);
#endif
	gtk_dialog_set_default_response(GTK_DIALOG(data->window), GTK_RESPONSE_OK);
	gtk_container_set_border_width(GTK_CONTAINER(data->window), BOX_SPACING);
	gtk_window_set_resizable(GTK_WINDOW(data->window), FALSE);
	gtk_box_set_spacing(GTK_BOX(GTK_DIALOG(data->window)->vbox), CONTAINER_BORDER_WIDTH);
    gtk_container_set_border_width(GTK_CONTAINER(GTK_DIALOG(data->window)->vbox), BOX_SPACING);
    ...
    gtk_container_add(GTK_CONTAINER(hbox), vbox);
#else
    /* lets put it in a scroll window */
    sw = gtk_scrolled_window_new (NULL, NULL);
    gtk_widget_show (sw);
    gtk_container_add (GTK_CONTAINER (GTK_DIALOG(data->window)->vbox), sw);
    gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (sw), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
    ...
    hbox = vbox = gtk_vbox_new(FALSE, 0);
    gtk_container_add (GTK_CONTAINER (viewport), hbox);
...
*/
	...
	*/
    gtk_table_set_col_spacings(GTK_TABLE(table), BOX_SPACING);
...
#ifdef GAIM_SMALL_SCREEN
    gtk_entry_set_width_chars(data->entry, ENTRY_SIZE);
#endif
    gtk_table_attach (GTK_TABLE (table), data->entry, 1, 2, 0, 1,
                        (GtkAttachOptions) (GTK_FILL),
                        (GtkAttachOptions) (0), 0, 0);
...
#ifdef GAIM_SMALL_SCREEN
    gtk_entry_set_width_chars(data->entry_for_alias, ENTRY_SIZE);
#endif
    gtk_table_attach (GTK_TABLE (table), data->entry_for_alias, 1, 2, 1, 2,
                        (GtkAttachOptions) (GTK_FILL),
                        (GtkAttachOptions) (0), 0, 0);
...
#ifdef GAIM_SMALL_SCREEN
    gtk_widget_set_size_request(GTK_WIDGET(data->combo), 100, -1);
#endif
    gtk_combo_set_popdown_strings(GTK_COMBO(data->combo), groups_tree());
    
    gtk_table_attach (GTK_TABLE (table), data->combo, 1, 2, 2, 3,
                        (GtkAttachOptions) (GTK_FILL),
                        (GtkAttachOptions) (0), 0, 0);
    ...
    gtk_table_attach (GTK_TABLE (table), data->account_box, 1, 2, 3, 4,
                        (GtkAttachOptions) (0),
                        (GtkAttachOptions) (0), 0, 0);
								...
								const char *alias, const char *name)
{
    ...
    gtk_window_set_modal (GTK_WINDOW (data->window), TRUE);
#ifdef ENABLE_HILDON
    gtk_widget_set_size_request(GTK_WINDOW (data->window), 500, 400);
#endif
#endif
	...
	data->entries_box = gtk_vbox_new(FALSE, CONTAINER_BORDER_WIDTH);
	gtk_container_set_border_width(GTK_CONTAINER(data->entries_box), 0);
