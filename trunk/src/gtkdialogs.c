...
static struct translator current_translators[] = {
	...
	{N_("Albanian"),            "sq", "Programe Shqip", "besnik@spymac.com"},
...
void gaim_gtkdialogs_about(GtkWidget *w, void *data)
{
	...
	gtk_window_set_default_size(GTK_WINDOW(about), 480, 320); /* Golden ratio in da hizzy */
#else
...
gaim_gtkdialogs_remove_contact(GaimContact *contact)
{
	...
	if (((GaimBlistNode*)contact)->child == (GaimBlistNode*)buddy &&
		...
		gaim_request_action(contact, NULL, _("Remove Contact"), text, 
				0, contact, 2,
...
gaim_gtkdialogs_remove_buddy(GaimBuddy *buddy)
{
	...
	gaim_request_action(buddy, NULL, _("Remove Buddy"), text, 
						0, buddy, 2,
...
gaim_gtkdialogs_remove_chat(GaimChat *chat)
{
	...
	name = gaim_chat_get_display_name(chat);
